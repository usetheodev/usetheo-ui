...
describe("spanOwnCostUsd", () => {
  it("test_span_sem_custo_retorna_undefined_nunca_zero", () => {
    expect(spanOwnCostUsd(span({}))).toBeUndefined();
  });

  it("test_costUsd_positivo_retorna_o_valor", () => {
    expect(spanOwnCostUsd(span({ costUsd: 0.0234 }))).toBe(0.0234);
  });

  it("test_costUsd_zero_ou_negativo_retorna_undefined", () => {
  ...
  it("test_le_atributo_de_custo_quando_field_ausente", () => {
    expect(spanOwnCostUsd(span({ attributes: { "gen_ai.usage.cost": "0.5" } }))).toBe(0.5);
  });

  it("test_atributo_zero_retorna_undefined", () => {
...
describe("spanCostUsd / aggregateCost stay honest-zero", () => {
  it("test_spanCostUsd_ausente_colapsa_para_zero", () => {
    expect(spanCostUsd(span({}))).toBe(0);
    expect(spanCostUsd(span({ costUsd: 0 }))).toBe(0);
  });

  it("test_aggregateCost_soma_zero_nunca_undefined", () => {
